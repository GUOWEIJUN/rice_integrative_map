...
disableWGCNAThreads()
#unableWGCNAThreads()支持多线程，因服务器不支持，所以用disableWGCNAThreads()
#先用合并的做
RNAseq_voom <- read.delim('all.fpkm-merge_DS_HS_SS.txt',header=T,row.names = 1,sep="\t",na.strings = "-")

#======为了方便演示，只取前2000个基因作为测试==========
#实际分析中此句不执行
...
table(KeepSample)  #过滤的样品数目统计  （KeepSample TRUE 18）
...
table(KeepGenes)# 过滤统计（KeepGenes FALSE  TRUE 1732 25923）

datExpr=datExpr[, KeepGenes] #过滤基因
GeneName=colnames(datExpr) #过滤的基因名称
...
# 绘制一条设定的阈值线，如果没有超过红线的，说明没有找到软阈值，sft$powerEstimate为空
...
#===============自动构建WGCNA模型==================
#sft$powerEstimate=10    #如果前面没有找到软阈值，需要手动设置（具体数值根据上图决定）
...
net = blockwiseModules(
  ...
  maxBlockSize = 35700,                  #最大block大小，将所有基因放在一个block中
  TOMType = "unsigned",                  #选择unsigned，使用标准TOM矩阵
  deepSplit = 2, minModuleSize = 100,     #剪切树参数，deepSplit取值0-4
...
#输出分类的表格
write.table(table(moduleColors),file = "ME_class.txt",sep = "\t",quote = F)
...
write.table(MEs,file = "ME.corr.pearson.xls",sep = "\t",quote = F)

pdf("wgcna.adjacency.heatmap_tree.pdf",height = 10,width = 9)
plotEigengeneNetworks(MEs, "Eigengene adjacency heatmap",plotDendrograms = T,
                      marDendro = c(4,4,2,4))
dev.off()
#绘制所有模块的表达值热图与特征值条形图
for(module in substring(colnames(MEs),3)){
...
#========================从此处往下===========================
#===================模块与表型的关联分析======================
allTraits<-read.delim('Pheno.txt',header=T,sep="\t")  #读取表型值
...
#绘制关联热图
png("wgcna.Module-trait.heatmap.png", height = 1000,width = 900)
...
dev.off()
pdf("wgcna.Module-trait.heatmap.pdf", height = 15,width = 9)
par(mar = c(6, 8.8, 3, 2.2))  
labeledHeatmap(Matrix = moduleTraitCor,   #相关系数
               xLabels = colnames(datTraits), #x轴为表型
               yLabels = names(MEs),#y轴为模块
